...
mod tests {
    ...
    fn the_player_lifts_out_of_its_row() {
        ...
        // And the same speaker with its artwork up, which is what re-opening
        // one that is already playing looks like - the ordinary case, not an
        // edge one. There is no background to fall to, so the panel crosses
        // to the photograph band by band instead; what travels and the row's
        // card are untouched.
        let art = {
            let mut buffer = slint::SharedPixelBuffer::<slint::Rgb8Pixel>::new(W as u32, H as u32);
            let (w, stride) = (W, W);
        ...
        app.set_player_fanart(art);
        app.set_player_has_art(true);
        app.set_player_ready(true);
        app.set_player_connected(true);
        app.set_player_title("Weird Fishes / Arpeggi".into());
        app.set_player_metadata("Radiohead · In Rainbows".into());
        app.set_player_elapsed("1:14".into());
        app.set_player_remaining("−4:03".into());
        app.set_player_progress(24.0);
        let mut c = vec![slint::Rgb8Pixel::default(); W * H];
        settle();
        draw(&mut c);
        let with_art = word(&c);
        let plan = crate::player_plan(&app, from, W as i32, H as i32)
            .expect("a player showing artwork still lifts, band by band");
        assert_eq!(
            plan.crossing,
            crate::panel::Crossing::Banded,
            "a page with a photograph behind it fell to a background it has not got"
        );
        let mut art = crate::lift_art(&leaving, &with_art, W, H, plan);
        let content = crate::panel::lift_content(&leaving, W, H);
        let screen_content = crate::panel::lift_content(&with_art, W, H);
        ...
        let (mean, worst) =
            crate::panel::checks::profile(&leaving, &with_art, W, H, plan, "player banded");
        assert!(
            mean <= 0.25 && worst <= 0.35,
            "the banded lift costs {mean:.2} panels a frame on average and {worst:.2} at its \
             worst - a photograph crossed too much of the panel at once"
        );
        crate::panel::checks::never_bare(&leaving, &with_art, W, H, plan, "player banded");
        crate::panel::checks::lands(&leaving, &with_art, W, H, plan, "player banded");
        crate::panel::checks::stronger(&leaving, &with_art, W, H, plan, "player banded");
        crate::panel::checks::ghosts(&leaving, &with_art, W, H, plan, "player banded");
        crate::panel::checks::pops(&leaving, &with_art, W, H, plan, "player banded");
        app.hide().unwrap();
        let _ = std::fs::remove_file(&path);
    }
