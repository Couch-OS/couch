...
try {
  ...
  await appearance.getByRole('alert').filter({hasText:'Use a color'}).waitFor();
  assert.equal((await config()).appearance.accent,'#B794F4');
  const rejected=await context.request.put(`${origin}/api/appearance`,{data:{accent:'#bad'}});assert.equal(rejected.status(),422);
  assert.equal((await config()).appearance.accent,'#B794F4');
