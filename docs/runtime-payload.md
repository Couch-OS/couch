...
| Bluetooth bridge | `couch-bt-bridge`, built in the clients workspace (`-p couch-bt`); static ARM32 ELF. Not in the runtime bundle yet: updaters before .142 refuse a bundle with a file they do not list, so it rides in the boot ramdisk (`/extra/couch-bt-bridge`, `prepare_boot_candidates.clean_ramdisk`) beside the kernel that gives it `/dev/vhci`. The system service starts it only while the Bluetooth setting is on, looking beside itself first, then in `/extra`, then `/opt/couch` |
