...
- Range 60000..=64999, gid = uid. A removed package keeps its row, so a number is not recycled while the range lasts; the only two things that reuse one are the range running out (the rows of packages no longer in `state/` are dropped and the lowest free number is used) and a table too damaged to read (rebuilt deterministically from the sorted ids in `state/`, which the daemon is told about so it retires its children). A hash was rejected (collisions; a custom-repo package could pick an id to collide with a victim). Package ids are unique per store.
- Allocation: `Store::identity_wait(id, wait) -> Result<(u32,u32)>` (`identity` = the store-mutation bound; `resolve_with_identity` reads the version and the user under one shared lock for the request path, bounded by what is left of the request). Assigned in `admit_expected` (lib.rs:411) at install and by one `assign_identities()` pass in `Runtime::new` for what is installed. A missing/corrupt table is rebuilt (safe: NOTHING on disk is owned by these ids). A busy store => `Error::is_busy()` and the request is retried by the next key press. The range genuinely full (every number held by a package still installed) => `Error::is_out_of_users()`, which is NOT busy and reaches HTTP as `Invalid`: waiting changes nothing, the person has to remove a package. A table that cannot be written leaves the number in memory for the life of the process and logs it. Never, in any case, a silent fallback to 65534.
